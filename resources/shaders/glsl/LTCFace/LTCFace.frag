...
void main() {
    ...
    vec2 uv = vec2(materialUbo.roughness, sqrt(1.0 - nDotV)); //使得cos接近1时密度大
    ...
    vec3 points[4];
    float halfH = lightUbo.height * 0.5;
    float halfW = lightUbo.width * 0.5;

    points[0] = vec3(-halfW, -halfH, 0.0);
    points[1] = vec3(-halfW,  halfH, 0.0);
    points[2] = vec3( halfW,  halfH, 0.0);
    points[3] = vec3( halfW, -halfH, 0.0);


    // 变换到世界空间
    for(int i = 0; i < 4; i++){
        vec4 worldPos = lightUbo.lightMatrix * vec4(points[i], 1.0);
    ...
    vec3 radiance = lightUbo.color * lightUbo.intensity;
    vec3 col = radiance * (spec + materialUbo.diffuse * diff);

    //Debug
//    vec3 dir = points[0].xyz - outWorldPos;
//    vec3 lightnormal = cross(points[1] - points[0], points[3] - points[0]);
