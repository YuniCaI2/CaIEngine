    ...
    // 创建Vulkan资源
     ...
     auto presentPass = renderPassManager.RegisterRenderPass([&](auto& renderPass) {
         ...
         renderPass->SetExec([&](VkCommandBuffer cmdBuffer) {
             ...
             FrameWork::CaIMaterial::Get(presentMaterial)->SetTexture("colorTexture", colorAttachment);
     ...
     renderPassManager.FindRenderPass(presentPass)->SetCreateResource(swapChainAttachment).SetReadResource(colorAttachment);
